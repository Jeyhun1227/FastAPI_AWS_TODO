...
const Home: NextPage = () => {
  const userId: string = "anhduy1202";
  ...
  const getTasks = async () => {
    setIsLoading(true);
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/list-tasks/${userId}`);
  ...
  const putTask = async (task: Task) => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/create-task`, {
      method: "PUT",
      headers: {
  ...
  const updateTask = async (updatedTask: Task) => {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/update-task`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
